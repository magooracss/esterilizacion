...
type
  ...
  public
    procedure CargarComboBox (var elComboBox: TComboBox; campoVisible, campoIndice: string; var tablaRelleno: TZQuery);
     ...
     procedure IntervaloHorasConsulta  (var hIni, hFin: TDateTime);


     procedure LevantarReporte (Reporte: string; elDataset: TDataSet);
     procedure LevantarReporteEsp (Reporte: string; elDataset: TDataSet; var refReporte: TfrReport);
     procedure EditarReporte;
     procedure EjecutarReporte;
     procedure EjecutarReporteSilencioso;
     procedure EjecutarReporteSilenciosoEsp(var refReporte: TfrReport);
...
(*
FunciÃ³n general que recibe un comboBox y una tabla y devuelve el combo con los valores cargados y
...
del combo donde estÃ¡ esa clave. Si no la encuentra, devuelve -1
*)
...
procedure TDM_General.IntervaloHorasConsulta(var hIni, hFin: TDateTime);
var
 h,m,s,ml: Word;
begin
  DecodeTime(hIni,h,m,s,ml);
  hIni:= EncodeTime(h,m,0,0);

  DecodeTime(hFin,h,m,s,ml);
  hFin:= EncodeTime(h,m,0,0);
end;
...
procedure TDM_General.LevantarReporteEsp(Reporte: string; elDataset: TDataSet;
  var refReporte: TfrReport);
var
  ruta: string;
begin
  with refReporte do
...
procedure TDM_General.EjecutarReporteSilenciosoEsp(var refReporte: TfrReport);
begin
  refReporte.ModalPreview:= False;
  refReporte.PrepareReport;
  refReporte.PrintPreparedReport('1',1);
end;
...
{
 determina si el dÃ­gito verificador es correcto
